...
indicators_db <- function(sheet_names) {
  ...
  # Read all files
   ...
   openxlsx::write.xlsx(All
                        , here("2025_RF_indicators", "indicators_db-V0.3.xlsx")
                        , sheetName = names(All)
                        , overwrite = TRUE)
   
   invisible(gc(verbose = FALSE, reset = TRUE)) # Collecting garbage after each iteration
  }
